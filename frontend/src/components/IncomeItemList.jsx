...
const IncomeItemList = ({
    ...
    useEffect(() => {
        const filtered = incomes.flatMap((income) =>
            income.product_markings
                ? income.product_markings.filter(
                    (marking) =>
                        !marking.outcome && // Проверяем, что маркировка не имеет outcome
    ...
    }, [searchTerm, incomes, onUpdateMarkingCount, products]);

    const handleMarkingChange = async (markingId, newMarking) => {
        if (!selectedMarking) return;

        try {
            await updateMarking(selectedMarking.incomeId, selectedMarking.productId, markingId, newMarking);
            setIncomes((prevIncomes) =>
                prevIncomes.map((income) => ({
                    ...income,
                    product_markings: income.product_markings.map((marking) =>
                        marking.id === markingId ? {...marking, marking: newMarking} : marking
                    ),
                }))
            );
        } catch (error) {
    ...
    const handleMarkingDelete = async (markingId) => {
        ...
        try {
            ...
            setIncomes((prevIncomes) =>
                prevIncomes.map((income) => ({
                    ...income,
                    product_markings: income.product_markings.filter((marking) => marking.id !== markingId),
                }))
            );
            closeModal();
        } catch (error) {
