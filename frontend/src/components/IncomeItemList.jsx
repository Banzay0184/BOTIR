...
import {updateMarking, deleteMarking, getProducts} from '../api/api'; // Предполагается, что вы добавили функцию deleteMarking
...
const IncomeItemList = ({
    ...
    const handleDeleteMarking = async (marking) => {
        console.log('Delete button clicked', marking); // Проверка клика

        try {
            await deleteMarking(marking.incomeId, marking.productId, marking.id);
            console.log('Marking deleted successfully'); // Проверка успешного удаления

            setIncomes((prevIncomes) =>
                prevIncomes.map((income) => ({
                    ...income,
                    product_markings: (income.product_markings || []).filter(
                        (item) => item.id !== marking.id
                    ),
                }))
            );
        } catch (error) {
                  ...
                  >
                    ...
                    <td className="p-4">
                        ...
                        <button
                            ...
                            onClick={() => handleDeleteMarking(marking)}
                        >
