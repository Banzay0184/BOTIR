    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setIsLoading(true);
                setError(null);
                const response = await getIncomeById(id, controller.signal);
                ...
                <section className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-blue-gray-800">
                        <div>
                            ...
                            <p><span className="text-blue-gray-500">Номер контракта:</span> {income.contract_number}</p>
                            <p><span className="text-blue-gray-500">Дата контракта:</span> {income.contract_date}</p>
                            <p><span className="text-blue-gray-500">Номер счета:</span> {income.invoice_number}</p>
                            <p><span className="text-blue-gray-500">Дата счета:</span> {income.invoice_date}</p>
                        </div>
                        <div>
                            <h2 className="text-sm font-semibold text-blue-gray-500 uppercase tracking-wide mb-2">Компания</h2>
                            <p><span className="text-blue-gray-500">Название:</span> {income.from_company?.name ?? '—'}</p>
                            <p><span className="text-blue-gray-500">ИНН:</span> {income.from_company?.inn ?? '—'}</p>
                            <p><span className="text-blue-gray-500">Телефон:</span> {income.from_company?.phone ?? '—'}</p>
                        </div>
                    </div>
                            ...
                            <table className="table-auto w-full text-left">
                                ...
                                <tbody className="text-blue-gray-800 divide-y divide-blue-gray-100">
                                    {productDetails.length > 0 ? (
                                        productDetails.map((detail, index) => (
                                            <tr key={detail.id} className="hover:bg-blue-gray-50/50">
                                                ...
                                                <td className="px-4 py-3">{income.unit_of_measure ?? '—'}</td>
                                    ...
                                    <tr className="bg-blue-gray-50/80 font-semibold">
                                        ...
                                        <td className="px-4 py-3 text-right">{income.total != null ? income.total.toLocaleString() : '—'} сум.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
export default IncomeDocumentViewPage;
